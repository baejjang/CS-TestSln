...
namespace ExamAsyncDelegate
{
     ...
     * BeginInvoke()는 쓰레드를 구동시킨 후 IAsyncResult 객체를 리턴하고 즉시 메인쓰레드의
     * 다음 문장을 실행하게 된다. IAsyncResult 객체는 차후 EndInvoke() 등과 같은 메서드를
     * 실행할 때 파라미터로 전달되는 것으로서 어떤 쓰레드를 가리키는 지를 알 수 있게 한다.
     */
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            for (int i = 0; i < 20; i++ )
            {
                Console.WriteLine("Do something1 in Main thread"+i);
            }
            ...
            for (int i = 0; i < 50; i++)
            {
                Console.WriteLine("Do something2 in Main thread"+i);
            }
